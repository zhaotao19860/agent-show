...
自动识别平台、抓最新 release、校验 SHA-256，把二进制放到 `~/.local/bin`（或 `/usr/local/bin`，若可写）。环境变量：`AGENT_SHOW_VERSION=v1.0.0` 锁版本，`AGENT_SHOW_PREFIX=/opt/bin` 改安装目录。
...
下载最新 release、校验 SHA-256、安装到 `%LOCALAPPDATA%\agent-show` 并加入用户 PATH。锁定版本：`$env:AGENT_SHOW_VERSION="v1.9.1"`。
...
从 [Releases](https://github.com/benjamin7007/Agent Show/releases/latest) 抓最新版：
...
- **单一二进制** —— `agent-show-server`（axum）通过 `rust-embed` 在 build 阶段把 React 19 SPA 一并嵌入，运行时无需单独的静态文件目录。
