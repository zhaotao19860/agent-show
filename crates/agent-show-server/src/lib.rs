...
pub struct AppState {
    ...
    pub events: broadcast::Sender<agent_show_core::SessionEvent>,
...
pub fn spawn_watcher(state: AppState) {
    ...
    tokio::spawn(async move {
        ...
        while let Some(ev) = m_rx.recv().await {
            match &ev {
                agent_show_core::SessionEvent::DetailUpdated { session_id, .. }
                | agent_show_core::SessionEvent::Closed { session_id } => {
                ...
                agent_show_core::SessionEvent::ConversationUpdated { .. } => {}
                agent_show_core::SessionEvent::SessionListChanged => {
                    rcache.invalidate("overview").await;
